package punk.fx.graphics 
{
	...
	/**
	 * An extended PreRotation class to which FXs can be applied.
	 * 
	 * @author azrafe7
	 */
	public class FXPreRotation extends PreRotation implements IFXGraphic 
	{
		
		/** @private Auto-incremented counter for id. */
		protected static var _idAutoCounter:int = 0;
		
		/** @private Id of the FXPreRotation. */ 
		...
		/** @private Callback function called just before rendering (signature: <code>function(img:FXPreRotation)</code>). */
		...
		/**
		 * Creates a new FXPreRotation with the specified parameters, and assigns an id to it.
		 * 
		 ...
		 */
		public function FXPreRotation(source:Class, frameCount:uint = 36, smooth:Boolean = false) 
		{
			super(source, frameCount, smooth);
			
			active = true;					// sets the FXPreRotation to active (set this to false if you don't want the FXPreRotation to update)
			autoUpdate = true;				// updates the buffer every frame
			_id = _idAutoCounter++;
		}
		
		/**
		 * Renders the FXPreRotation.
		 * 
		 * @param	target		where to draw the FXPreRotation.
		...
		/**
		 * Callback function called just before rendering (signature: <code>function(img:FXPreRotation)</code>).
		 */
