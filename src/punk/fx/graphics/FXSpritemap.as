package punk.fx.graphics 
{
	...
	/**
	 * An extended Spritemap class to which FXs can be applied.
	 * 
	 * @author azrafe7
	 */
	public class FXSpritemap extends Spritemap implements IFXGraphic 
	{
		
		/** @private Auto-incremented counter for id. */
		protected static var _idAutoCounter:int = 0;
		
		/** @private Id of the FXSpritemap. */ 
		...
		/** @private Callback function called just before rendering (signature: <code>function(img:FXSpritemap)</code>). */
		...
		/**
		 * Creates a new FXSpritemap with the specified parameters, and assigns an id to it.
		 * 
		 ...
		 */
		public function FXSpritemap(source:*, frameWidth:uint=0, frameHeight:uint=0, callback:Function=null) 
		{
			super(source, frameWidth, frameHeight, callback);
			
			active = true;					// sets the FXSpritemap to active (set this to false if you don't want the FXSpritemap to update)
			autoUpdate = true;				// updates the buffer every frame
			_id = _idAutoCounter++;
		}
		
		/**
		 * Renders the FXSpritemap.
		 * 
		 * @param	target		where to draw the FXSpritemap.
		...
		/**
		 * Callback function called just before rendering (signature: <code>function(img:FXSpritemap)</code>).
		 */
